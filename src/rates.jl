...
# Arguments
 ...
 - `gam_flanking::Array{Int64,1}` : Range of deleterious selection coefficients at flanking region
 - `gam_dfe::Array{Int64,1}` : Range of deleterious selection coefficients at coding region
...
function rates(
	...
	for i::Int64 = 1:iterations
		push!(priors,[rand(u_gh),rand(u_gl),rand(u_gam_flanking),rand(u_gam_dfe),shape * 2^rand(afac),rand(u_tot),rand(u_low)])
	end

	# Solving models in multi-threading
	@info "Solving models in multiple threads"
	m, r_ps, r_pn, r_f = unzip(ThreadsX.map(x -> simulate_models!(deepcopy(param), binom, x), priors))
