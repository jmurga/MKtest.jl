...
function sfs_neut(param::parameters, binom::SparseMatrixCSC{Float64, Int64})
    ...
    out::Array{Float64, 1} = param.B * (param.θ_coding) * 0.25 *
                             (binom * solved_neutral_sfs)
...
"""
function sfs_pos(param::parameters, s::Int64, p::Float64,
    ...
    if p == 0.0
        ...
        function positive_sfs(i::Float64, g1::Float64 = s_exp1, g2::Float64 = s_exp2,
                              p::Float64 = p)
        ...
        out::Array{Float64, 1} = (param.θ_coding) * red_plus * 0.75 *
                                 (binom * solved_positive_sfs)
    end

    return out
end

function sfs_pos_float(param::parameters, s::Int64, p::Float64,
                       binom::SparseMatrixCSC{Float64, Int64})
    if p == 0.0
        out = zeros(Float64, param.nn + 1)
        out = out[2:(end - 1)]
        ...
        function positive_sfs(i::Float64, g1::Quadmath.Float128 = s_exp1,
                              g2::Quadmath.Float128 = s_exp2, p::Float64 = p)
            Float64(p * 0.5 * (g1 * (1 - g2^(1.0 - i)) / ((g1 - 1.0) * i * (1.0 - i))))
        end
        # Allocating outputs
        solved_positive_sfs::Array{Float64, 1} = (1.0 / (NN2)) * (positive_sfs.(xa2))
...
function sfs_neg(param::parameters, p::Float64, binom::SparseMatrixCSC{Float64, Int64})
    ...
    function z(x::Float64, p::Float64 = p)
        (1.0 - p) * (2.0^-param.shape) * (beta^param.shape) *
