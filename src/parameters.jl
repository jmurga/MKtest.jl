...
@with_kw mutable struct parameters
	...
	be::Float64                 = 0.000402
	B::Float64                  = 0.999 
	B_bins::Vector{Float64}     = push!(collect(0.1:0.025:0.975),0.999)
	ppos_l::Float64             = 0
...
Find the optimum mutation rate given the expected reduction in nucleotide diversity (B value) in a locus.

# Inputs
 - `param::parameters`: mutable structure containing the model
# Returns
 - `parameters.θ_noncoding::Float64`: inplace change on parameters.θ_noncoding value.
"""
...
function solve_eqns(param,config)
...
Find the probabilty of positive selected alleles given the model. It solves an equation system taking into account fixations probabilities of weakly and strong beneficial alleles.

# Input
 - `param::parameters`: mutable structure containing the model
# Returns
 - `Tuple{Float64,Float64}`: inplace change on weakly (parameters.ppos_l) and strong (parameters.ppos_l) beneficial alleles probabilites.
"""
...
# Returns
 - `Dict{Float64,SparseMatrixCSC{Float64,Int64}}`: convoluted SFS for each B value defined in the model (param.B_bins).
"""
...
Binomial convolution to sample the allele frequencies probabilites depending given population size, sample size and background selection value.
...
# Returns
 - `Vector{Float64}`: α(x).
 - `Vector{Float64}`: α(x) without weakly beneficial alleles.
"""
