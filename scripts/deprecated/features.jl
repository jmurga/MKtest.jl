...
function Gammadist(gamma)
    return ((adap.be^adap.al) / SpecialFunctions.gamma(adap.al)) * (gamma^(adap.al - 1)) *
           exp(-adap.be * gamma)
end
...
function pip0(gamma)
    ...
    return Gammadist(gamma) * exp(-(Gammadist(gamma) * U / (2.0 * adap.NN)) /
               (gamma / (adap.NN + 0.0) + R / (2.0 * adap.NN)))
end
...
function calcBGam(L, alpha, beta, theta)
    ...
    a = -1.0 * (2^(alpha + 1)) * exp(L * adap.NN * r * beta) * L * adap.N *
        ((1.0 / (L * adap.N * r))^(1 - alpha)) * u * (beta^alpha)
    b = convert(Float64,
                SpecialFunctions.gamma_inc(1 - alpha, L * adap.NN * r * beta, 1)[2])
...
function plotMap(; analysis::String, output::String)
    try
        ...
        maxp = DataFrame(Array{Float64, 2}(undef, size(posteriors, 1), 5),
                         [:aw, :as, :a, :gamNeg, :shape])
...
function poisson_fixation(; observed_values::Vector{S}, λds::Vector{Float64},
                          λdn::Vector{Float64}, λweak::Vector{Float64},
                          λstrong::Vector{Float64}) where {S <: Union{Int64, Float64}}
